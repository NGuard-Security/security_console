          ...
          <div v-if="connState == 1">
              ...
              <ul class="flex flex-col gap-4 h-full overflow-y-scroll">
                  <li v-for="member in memberList" class="flex items-center shrink-0 rounded-lg px-7 py-4 text-ellipsis whitespace-nowrap overflow-hidden box-content">
                      ...
                      <button v-if="!member.isBlackList" class="text-sm p-1.5 rounded-lg bg-zinc-900 ml-auto" @click="setBlackList(member.id)">
                          블랙리스트
                      </button>
                      <button v-else class="text-sm text-red-500 p-1.5 rounded-lg ml-auto" style="background: rgb(56, 23, 23);" @click="setBlackList(member.id)">
                          블랙리스트 해제
                      </button>
                  </li>
              </ul>
