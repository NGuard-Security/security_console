  ...
  <main class="max-w-6xl flex flex-col">
      ...
      <div v-if="connState == 1">
        ...
        <ul class="flex flex-col gap-4 h-full overflow-y-scroll">
          ...
          >
            ...
            <img
              :src="member.icon + '?size=128'"
              alt="user_logo"
              class="h-8 mr-2 rounded-lg"
            />

            <span
              class="mr-1 text-lg min-w-0 overflow-hidden text-ellipsis"
              >{{ member.nickName }}</span
            >
            <p
              class="min-w-0 mr-4 overflow-hidden text-ellipsis text-gray-400"
            >
              {{ member.userName }}
            </p>
    ...
    >
      ...
      <div class="text-gray-400 pt-5 pb-8">
        ⚠️ 블랙리스트 등록 후 일정 기간 동안은 등록을 취소 할 수
        있으나,<br />
    ...
    <modal
      class="modal sureRemoveBlackList"
      name="sureRemoveBlackList"
      width="500"
    >
      <h2>
