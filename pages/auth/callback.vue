        ...
        <div class="overflow-x-hidden servers-wrap flex items-center justify-center flex-col w-full h-full min-h-screen px-10">
            ...
            <div class="relative w-full flex justify-center mb-20">
                ...
                <div id="loading" class="absolute mx-auto">
                    ...
                    <h4 class="text-base pt-5" style="color: rgb(153, 153, 153)">로그인 정보를 불러오는 중입니다...</h4>
                    </div>
                </div>
                </transition>
            </div>
...
<script>
  export default {
    async mounted() {
        try {
            const login = await this.$axios.$post('http://127.0.0.1:4000/auth/callback', { // Production: API 서버 주소로 바꾸기 (eg. https://api.nguard.xyz/~~~ )
